...
function SocialIconLink({ label, href, children }: SocialIconProps) {
  return (
    <a
      ...
      aria-label={label}
      title={label}
      className="flex h-12 w-12 items-center justify-center rounded-full border border-border text-muted transition-all hover:border-orange/40 hover:bg-orange/10 hover:text-foreground sm:h-14 sm:w-14"
    >
      {children}
    </a>
  );
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-3 sm:gap-5">
              {footerLinks.social.map((link) => {
                ...
                return (
                  <SocialIconLink key={link.label} label={link.label} href={link.href}>
                    {Icon ? <Icon /> : null}
                  </SocialIconLink>
                );
              })}
